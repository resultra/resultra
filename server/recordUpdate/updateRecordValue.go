...
func UpdateRecordValue(appEngContext appengine.Context, recUpdater RecordUpdater) (*record.RecordRef, error) {

	recordID := record.RecordID{recUpdater.recordID()}
	...
	recordRefForUpdate, getErr := record.GetRecord(appEngContext, recordID)
	if getErr != nil {
	...
	recUpdater.updateRecordValue(&recordRefForUpdate.FieldValues)
	...
	if calcErr := calcField.UpdateCalcFieldValues(appEngContext, recordRefForUpdate); calcErr != nil {
	...
	updatedRecordRef, updateErr := record.UpdateExistingRecord(appEngContext, recordID, recordRefForUpdate.FieldValues)
	if updateErr != nil {
