...
type NewSortRuleParams RecordSortRule
...
func saveFormSortRules(params NewSortRuleParams) (*FormSortRules, error) {
	...
	for _,sortRule := range params.SortRules {
		if !validSortDirection(sortRule.SortDirection) {
			return nil, fmt.Errorf("saveSortRule: Invalid sort direction '%v'",params.SortDirection)
		}		
	...
	if encodeErr != nil {
		return fmt.Errorf("saveSortRule: Unable to encode sort rules %+v: error = %v", params.SortRules, encodeErr)
	}
	...
	sortRules := FormSortRules {
		...
		SortFieldID:params.SortRules}

	log.Printf("saveSortRule: Saved sort rule: %+v", sortRule)

	return &sortRules, nil
}

func GetFormSortRules(parentFormID string) ([]RecordSortRule,error) {
	...
	var formSortRules FormSortRules
	encodedSortRules := ""
