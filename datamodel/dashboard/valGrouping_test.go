...
func newTestNumField(appEngContext appengine.Context, t *testing.T, refName string) string {
	...
	testNumField := datamodel.NewFieldParams{Name: refName, Type: datamodel.FieldTypeNumber, RefName: refName}
	testNumFieldID, numFieldErr := datamodel.NewField(appEngContext, testNumField)
	if numFieldErr != nil {
		t.Fatal(numFieldErr)
	}
...
func newTestTextField(appEngContext appengine.Context, t *testing.T, refName string) string {
	...
	testField := datamodel.NewFieldParams{Name: "Test Text Field", Type: datamodel.FieldTypeText, RefName: "TestRef1"}
	testFieldID, err := datamodel.NewField(appEngContext, testField)
	if err != nil {
		t.Fatal(err)
	}
