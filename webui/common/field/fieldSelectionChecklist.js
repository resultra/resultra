function initFieldSelectionChecklist(params) {
	
	var $fieldSelectionList = $('#' + params.elemPrefix + 'FieldSelectionChecklist')
	
	var defaultFieldsLookup = new IDLookupTable(params.defaultFields)
	
	
	var $label = $('#' + params.elemPrefix + 'FieldChecklistSelectionLabel')
	...
	function addFieldSelection(fieldInfo) {
		...
		var fieldEnabled = defaultFieldsLookup.hasID(fieldInfo.fieldID)
		initCheckboxControlChangeHandler($selectionCheckbox,fieldEnabled,function(isEnabled) {
			...
			params.setFieldsCallback(checkedFields)
		})
